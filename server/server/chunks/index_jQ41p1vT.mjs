...
import { z as handlePluginGet } from "./query-Di7DOmPV_CieW2RCL.mjs";
...
//#region node_modules/emdash/dist/astro/routes/api/admin/plugins/_id_/index.mjs
...
var GET = async ({ params, locals }) => {
	...
	const result = await handlePluginGet(emdash.db, emdash.configuredPlugins, emdash.sandboxedPluginEntries, id, emdash.config.marketplace);
	if (result.success) {
		const tools = await emdash.getPluginMcpTools(id);
		result.data.item.mcpTools = tools.map(({ inputSchema: _, outputSchema: __, pluginId: ___, ...tool }) => tool);
	}
	return unwrapResult(result);
};
//#endregion
//#region \0virtual:astro:page:node_modules/emdash/dist/astro/routes/api/admin/plugins/_id_/index@_@mjs
